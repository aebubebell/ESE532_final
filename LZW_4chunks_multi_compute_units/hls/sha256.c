...
{
    uint8_t *content = (uint8_t*)Chunk;
    uint32_t state[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
    
    int remain = chunk_length%64;
    int last = chunk_length-remain;

    if(chunk_length > 64)
    {
        sha256_process_arm(state,content,last);
    }

    uint64_t chunk_length_bit = remain *8;
    int factor = 64 - remain;
    int multiple = chunk_length + factor;
    ...
    int pad_size = multiple -chunk_length -9;
    uint8_t pad[pad_size]={0};
    uint8_t id= 1<<7;
    uint8_t data[128];

    int offset =0;
    memcpy(data,&content[last],remain);
    offset += remain;
    memcpy(data+offset,&id,sizeof(uint8_t));
    offset += sizeof(uint8_t);
    memcpy(data+offset,pad,pad_size);
    offset += pad_size;

    for(int i=0;i<8;i++)
    {
        uint8_t current = chunk_length >>((7-i)<<3);
        memcpy(data+offset+i,&current,sizeof(uint8_t));
        offset +=sizeof(uint8_t);
    }
    sha256_process_arm(state,data,offset);
    std::string hash;
    for(int i=0;i<8;i++)
    {
