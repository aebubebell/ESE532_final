...
*/
void LZWencoding(unsigned char* Chunk, int* encode_array,unsigned int chunk_length,int& compress_length)
{
    ...
    for(unsigned int i=0;i<chunk_length;i++) //if there is something in current Chunk pointer
    {
        char C = *(Chunk+i);//char C equal to current pointer 
